...
void DoConverCallback(void *data, char * str, long status) {
    ...
    if(env)
    {
        ...
        jmethodID javaOnEventMethodID = env->GetMethodID(jEpubDocumentClass, "onEvent","(ILjava/lang/String;)V");
...
(JNIEnv *env, jobject obj, jint width, jint height, jint mrginLeft, jint mrginTop, jint mrginRight, jint mrginBottom, jfloat density)
{
    SZEbookReader* bookReader = GetBookReader(env, obj);
...
(JNIEnv *env, jobject obj, jstring bookPath)
{
    ...
    if (re) {
        ...
        jfieldID fieldIssued = env->GetFieldID(jEpubInfoClass,"issued","Ljava/lang/String;");
        jfieldID fieldSummary = env->GetFieldID(jEpubInfoClass,"summary","Ljava/lang/String;");
        jfieldID fieldCategory = env->GetFieldID(jEpubInfoClass,"category","Ljava/lang/String;");
        jfieldID fieldPublisher = env->GetFieldID(jEpubInfoClass,"publisher","Ljava/lang/String;");
        jfieldID fieldAuthor = env->GetFieldID(jEpubInfoClass,"author","Ljava/lang/String;");
...
(JNIEnv *env, jobject obj)
{
    ...
    if (g_status == 1 || g_status == 3) {
        if (bookReader->bookreader.m_book->m_bookChapter->bHaveChapter()) {
            ...
            int i = 0;
            for (posChapter = orderChapter.begin(), posRange = pageRange.begin();posChapter != orderChapter.end();
                 posChapter++,posRange++) {
...
(JNIEnv *env, jobject obj, jint fileid, jint smallPageNumber, jobject bitmapObj)
{
    ...
    jbyte *bitmapData = (jbyte*) bookReader->GetPageByFileSmallPageNumber(fileid, smallPageNumber,bmp, lSize, pageOutData);
