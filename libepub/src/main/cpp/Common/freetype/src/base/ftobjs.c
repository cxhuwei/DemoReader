...
#define FT_COMPONENT  trace_objs
                 ...
                 FT_Int32  load_flags )
  {
	...
	{
		LOGD("FT_Err_Invalid_Face_Handle line:572");
    ...
    {
      //LOGD("call load_glyph line:692   face->size :%d glyph_index:%d load_flags : %d", face->size,glyph_index,load_flags);
	  error = driver->clazz->load_glyph( slot,
                                         face->size,
                                         glyph_index,
                                         load_flags );
      if ( error ){
        //LOGD("goto Exit  %0x: line:692",error);
		goto Exit;
		}
      if ( slot->format == FT_GLYPH_FORMAT_OUTLINE )
      {
        ...
        if ( error )
		{
		  LOGD("goto Exit  %0xd: line:700",error);goto Exit;
		}
#ifdef GRID_FIT_METRICS
