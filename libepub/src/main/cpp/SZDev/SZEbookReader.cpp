...
void SZEbookReader::initWithopp()
{
	...
	strcpy(m_strfontFilePath, "g:\\testdata\\fonts\\wqywmh.ttc");///Users/shaozhen/Library/Application Support/iPhone Simulator/5.0/Applications/5BBE4002-772C-48C5-9191-159804C66591/Control Fun.app/
	...
	color.rgbRed = 0xff;
	color.rgbGreen = 0xff;
	color.rgbBlue = 0x00;//黄
	m_vColor.push_back(color);
	color.rgbRed = 0xff;
	color.rgbGreen = 0x45;
	color.rgbBlue = 0x00;//橙
	m_vColor.push_back(color);
	color.rgbRed = 0x7f;
	color.rgbGreen = 0xff;
	color.rgbBlue = 0x00;//绿
	m_vColor.push_back(color);
	color.rgbRed = 0x9f;
	color.rgbGreen = 0xff;
	color.rgbBlue = 0xff;//蓝
	m_vColor.push_back(color);
	color.rgbRed = 0xff;
	color.rgbGreen = 0x83;
	color.rgbBlue = 0xfa;//粉
	m_vColor.push_back(color);
...
{ 
	...
	case 1:	
		...
		bInit = pImgScal->Init(pimage->GetBits(),lWidth,lHeight,1,bB0W1,newWidth,newHeight,1);
		break;
	case 8:
...
void SZEbookReader::readLastCalcPageRecord()
{
	...
	sprintf(filePath,"%s%dc%d %s %s %d %d %d %d %d %d %d %d %d.%s",m_currentPageRecordFileDir.c_str(),
            m_OutDevW,m_OutDevH,m_strfontname,m_strEfontname,m_ZoomRatio,GetMrginBottom(),GetMrginLeft(),
            GetMrginRight(),GetMrginTop(),GetTabSize(),GetVertSpac(),GetWordSpac(),m_calcVer,FileType);  
	...
	if (recordFile)
	{
		...
		while(fgets(buf,30,recordFile))
		{
			...
			m_AllPageNum++;
			if (myrecord.vfileID == m_currentRecord.vfileID && myrecord.contentNumber==m_currentRecord.contentNumber&&myrecord.offer==m_currentRecord.offer)
			{
    ...
    //LOGI("SZEbookReader_readLastCalcPageRecord: while end.");    
		...
		m_isCalcFinish = true;
        if(!(m_currentRecord.vfileID == 0 && m_currentRecord.contentNumber == 0 && m_currentRecord.offer == 0)) 
		{
			//得到当前页的首字信息
           long lSize;
...
void* SZEbookReader::GetPageByFileSmallPageNumber(unsigned int fileId,unsigned int smallPageNumber, SZImageBit24 &imageCur,long& imageSize,t_PageOutData& pageOutData)
{
    LOCK_MUTEX(&m_lockAllPageInfo);
    Map_myBookPageInfoIterator finder = m_mapAllPageInfo.find(fileId);
    UNLOCK_MUTEX(&m_lockAllPageInfo);
    
    if (finder != m_mapAllPageInfo.end() && finder->second.size() >= smallPageNumber && smallPageNumber > 0) {
...
unsigned int SZEbookReader::GetSmallPageNumberByRecord(myBookRecord record)
{
    ...
    if (!m_isCalcFinish)
    {
        ...
        {
            if (m_currenthtml != vfile[record.vfileID] || ((m_currenthtml == vfile[record.vfileID]) && m_vtCurrntFileInfo.size() == 0))
            {
...
int	SZEbookReader::calcCurrentHtml(int fileID,string filePath,bool isNeedRender,bool isCallbackProgress)
{
	...
	if (isNeedRender)
	{
		...
		if (RenderContentEx(m_vtCurrntFileInfo, m_currentvcontent,fileID,isCallbackProgress)!=1)
		{
			m_bookCalcCurWordNumber = 0;
			m_status=-83;
			return false;
...
bool SZEbookReader::calcOtherHtml()
{
	...
	{	
		...
		if (calcCurrentHtml(m_currentRecord.vfileID,vfile[m_currentRecord.vfileID],true,false)>0)
		{		
			int vtFileInfo=m_vtCurrntFileInfo.size();
			for(int i = 0;i < vtFileInfo; i++)
			{
				...
				if (i<(vtFileInfo-1))
				{
					if (m_lastBookRecord>m_vtCurrntFileInfo[i]&&m_lastBookRecord<m_vtCurrntFileInfo[i+1])
					{
						m_currentRecord=m_vtCurrntFileInfo[i];
						m_smallCurrentPageNum=i;
						break;
	...
	{		
		for(int i = 0;i < vfile.size(); i++)
		{
			...
			if (rt<=0)
			{
				...
				{
					strcpy(tttttinfo,"calcCurrentHtml error");
                    LOGI("SZEbookReader::calcOtherHtml | calcCurrentHtml error  :%d | path:%s", rt,m_currenthtml.c_str());
					CbkFunction(Cbkhost,tttttinfo,-1);
				}
				return false;
			}
	...
	{ 
		if (itor->first==m_currentRecord.vfileID)
		{
			...
			{				
				...
				m_AllPageNum+=1;
				if (tempRecord.contentNumber==m_currentRecord.contentNumber&&tempRecord.offer==m_currentRecord.offer)
				{
					m_CurrentPageNum=m_AllPageNum;
				}
			}
		...
		//保存计算后的信息
		//不需要判断记录文件里是否存在，因为能走到这个地方来，肯定是第一次计算。所以直接进行插入保存.
		...
		{	
			...
			{				
				tempRecord =itor->second[i];
				memset(item,0,30);
				sprintf(item,"%d %d %d\n",tempRecord.vfileID,tempRecord.contentNumber,tempRecord.offer);
...
void* SZEbookReader::ReadCorrespondPageByMuciIndex(int MuciIndex,SZImageBit24 &imageCur, long& imageSize,t_PageOutData& pageOutData)
{
			...
			{ 				
				...
				for (i=0;i<vfile.size();i++)
				{
					if (vfile[i]==infor.file)
					{
						...
						return GetPageByRecord(m_lastBookRecord, imageCur,imageSize,pageOutData,true);
					}
				}
				return NULL;
			}
...
void* SZEbookReader::GotoPage(int PageNum,SZImageBit24 &imageCur,long& imageSize,t_PageOutData& pageOutData,int flag)
{
		...
		{ 	
			...
			if (PageNum<=page)
			{
				...
				{	
					...
					if (page==PageNum)
					{
                        LOGI("SZEbookReader.GotoPage |find page | pageNum:%d | page:%d | i:%d | vtFileInfo:%d", PageNum, page, i,itor->second.size());
						m_CurrentPageNum=PageNum;
						m_vtCurrntFileInfo  =itor->second;
						m_currentRecord     =m_vtCurrntFileInfo[i];
						//vector<string>		vfile=bookreader.m_book->m_bookSpine->GetVSpine();
                        LOGI("SZEbookReader.GotoPage | find page | record:fileid:%d  contentNumber:%d  offset:%d ", m_currentRecord.vfileID, m_currentRecord.contentNumber, m_currentRecord.offer);
                        if (m_vtCurrntFileInfo.size() > 1) {
                             LOGI("SZEbookReader.GotoPage | m_vtCurrntFileInfo 0 :fileid:%d  contentNumber:%d  offset:%d ", m_vtCurrntFileInfo[0].vfileID, m_vtCurrntFileInfo[0].contentNumber, m_vtCurrntFileInfo[0].offer);
                            LOGI("SZEbookReader.GotoPage | m_vtCurrntFileInfo 1 :fileid:%d  contentNumber:%d  offset:%d ", m_vtCurrntFileInfo[1].vfileID, m_vtCurrntFileInfo[1].contentNumber, m_vtCurrntFileInfo[1].offer);
                        }
					...
					//	m_currentvcontent = bookreader.m_book->m_vContent;
                      ...
                      // 	long t1 = GetTickCount();
						...
						void* p = DrawCurrentPage(imageCur,imageSize,pageOutData,&m_currentRecord/*,&vContent*/);
	...
	if (1 == m_status)
	{
		...
		else
		{
			...
			*/
			...
			m_currentRecord.contentNumber = m_beforCalcRecord.contentNumber;
			m_currentRecord.offer = m_beforCalcRecord.offer;
			void* imgData = DrawCurrentPage(imageCur,imageSize,pageOutData,&m_currentRecord/*,&vContent*/);
...
void* SZEbookReader::PrevPage(SZImageBit24 &imageCur,long& imageSize,t_PageOutData& pageOutData)
{
	...
	{		
		...
		{	
			...
			void* temp2 = DrawCurrentPage(imageCur,imageSize,pageOutData,&drawPageRecord/*,&vContent*/);
			return temp2;
		}
	}	
	return NULL;
...
{ 
	...
	if (bmpHeader.biBitCount==1||bmpHeader.biBitCount==4||bmpHeader.biBitCount==8||bmpHeader.biBitCount==24)
	{
...
pageWordinfo SZEbookReader::GetCurPageWordInfo()
{
	for (int i=0;i < m_curPageWordInfo.lines.size();i++)
	{
		...
		{
			m_curPageWordInfo.lines[i].Width = m_curPageWordInfo.lines[i].words[m_curPageWordInfo.lines[i].words.size()-1].x + m_curPageWordInfo.lines[i].words[m_curPageWordInfo.lines[i].words.size()-1].width - m_curPageWordInfo.lines[i].x;
	...
	else
	{
		for (int i=0;i < m_curPageWordInfo.lines.size();i++)
		{
			m_curPageWordInfo.lines[i].x = (int)((double)m_curPageWordInfo.lines[i].x/ScreenRatio);
			m_curPageWordInfo.lines[i].y = (int)((double)m_curPageWordInfo.lines[i].y/ScreenRatio);
			m_curPageWordInfo.lines[i].Width = (int)((double)m_curPageWordInfo.lines[i].Width/ScreenRatio);
			m_curPageWordInfo.lines[i].Height = (int)((double)m_curPageWordInfo.lines[i].Height/ScreenRatio);
			for (int j=0;j < m_curPageWordInfo.lines[i].words.size();j++)
			{
				m_curPageWordInfo.lines[i].words[j].x = (int)((double)m_curPageWordInfo.lines[i].words[j].x/ScreenRatio);
				m_curPageWordInfo.lines[i].words[j].y = (int)((double)m_curPageWordInfo.lines[i].words[j].y/ScreenRatio);
				m_curPageWordInfo.lines[i].words[j].width = (int)((double)m_curPageWordInfo.lines[i].words[j].width/ScreenRatio);
				m_curPageWordInfo.lines[i].words[j].height = (int)((double)m_curPageWordInfo.lines[i].words[j].height/ScreenRatio);
			}
		}
		return m_curPageWordInfo;
	}
...
// 	thiscpn.e_offset = cpn.e_offset;
	...
	for (pos = m_curPageNotes.Notes.begin(); pos != m_curPageNotes.Notes.end(); pos++)
	{
		if (pos->status != 3)
		{
			SetHighLight3(imgCur,&pageRender,pos->s_contentID,pos->s_offset,pos->e_contentID,pos->e_offset,m_vColor[pos->colorIndex]);
		}
	}
...
bool SZEbookReader::DeleteCurPageNoteById(SZImageBit24& imgCur, int id )
{
	...
	int booknoteID(-1);
	Vt_curPageNotes::iterator pos;
	bool isHaveDelete(false);//脢脟路帽脮脪碌艙脡鸥鲁媒露脭脧贸
	for (pos = m_curPageNotes.Notes.begin(); pos != m_curPageNotes.Notes.end(); pos++)
	{
		if (pos->curBookId == id)
		{
			isHaveDelete = true;
			if (pos->status == 0 || pos->status == 2)
			{//脰庐脟掳卤拢沤忙碌脛脪陋卤锚艗脟脡鸥鲁媒拢卢碌卤禄禄脪鲁碌脛脢卤潞貌艙酶脨脨啪眉脨脗
				pos->status = 3;
			}
			else if (pos->status == 1)
			{//碌卤脟掳脪鲁脭枚艗脫碌脛脰卤艙脫脡鸥鲁媒
...
bool SZEbookReader::ModifyCurPageNoteById(SZImageBit24& imgCur,curPageNote& cpn)
{
	...
	int booknoteID(-1);
	Vt_curPageNotes::iterator pos;
	bool isFind(false);//脢脟路帽脮脪碌艙脨脼啪脛露脭脧贸
	for (pos = m_curPageNotes.Notes.begin(); pos != m_curPageNotes.Notes.end(); pos++)
	{
		if (pos->curBookId == cpn.curBookId)
		{
			isFind = true;
			if (pos->status == 0 || pos->status == 2)
			{//脰庐脟掳卤拢沤忙碌脛脪陋卤锚艗脟脨脼啪脛拢卢碌卤禄禄脪鲁碌脛脢卤潞貌艙酶脨脨啪眉脨脗//脮芒沤脦脭枚艗脫碌脛卤锚脳垄脠脭脦陋脭枚艗脫
				pos->status = 2;
			}
...
void* SZEbookReader::GotoInterPage(string url,SZImageBit24 &imageCur, long& imageSize,t_PageOutData& pageOutData)
{
	int page = -1;
	int i;
	vector<string>	vfile=bookreader.m_book->m_bookSpine->GetVSpine();
...
//打开图书之后，根据记录打开  需要isrender = false
void* SZEbookReader::GetPageByRecord(myBookRecord record,SZImageBit24 &imageCur, long& imageSize,t_PageOutData& pageOutData, bool isRender,bool isNeedDrawPage)
{
	...
	if (!m_isCalcFinish)
	{
		...
		return isNeedDrawPage?DrawCurrentPage(imageCur,imageSize,pageOutData,&m_currentRecord):NULL;
	}
	else
	{
		//已经计算完了
...
int SZEbookReader::digui(map<int,ChapterLevel>& orderchapter,map<int,ChapterLevel>::iterator& pos)
{
	...
	while (pos != orderchapter.end())
	{
		...
		{
			if (nextPos->second.level == pos->second.level)
			{//直接push
				startPageNumber = PushPageRangeIntoMap(startPageNumber,pos,nextPos,1);
			}
			else if (nextPos->second.level < pos->second.level)
			{//push后返回结束页
...
int SZEbookReader::PushPageRangeIntoMap(int nStartPageNumber,map<int,ChapterLevel>::iterator pos,map<int,ChapterLevel>::iterator nextPos,int isPosNull)
{
...
void SZEbookReader::SaveMuciRange()
{
	...
	sprintf(filePath,"%s%dc%d %s %s %d %d %d %d %d %d %d %dmuci %d.%s",m_currentPageRecordFileDir.c_str(),
		m_OutDevW,m_OutDevH,m_strfontname,m_strEfontname,m_ZoomRatio,GetMrginBottom(),GetMrginLeft(),
			GetMrginRight(),GetMrginTop(),GetTabSize(),GetVertSpac(),GetWordSpac(),m_calcVer,FileType);  
		...
		if (m_curPageWordInfo.fileID == temp.fileId)
		{
			for (int i = 0 ; i < m_curPageWordInfo.lines.size();i++)
			{
				...
				for (int j = 0;j<line.words.size();j++)
				{
					...
					if (wi.contentID >= temp.s_contentID && wi.contentID <= temp.e_contentID && wi.offset >= temp.s_offset && wi.offset <= temp.e_offset)
					{
						curPageNote curnote;
...
{
	//add  生成页码信息文件名
	sprintf(m_pageInfoFileName,"%s%dc%d %s %s %d %d %d %d %d %d %d %d %d.%s",m_currentPageRecordFileDir.c_str(),
		m_OutDevW,m_OutDevH,m_strfontname,m_strEfontname,m_ZoomRatio,GetMrginBottom(),GetMrginLeft(),
            GetMrginRight(),GetMrginTop(),GetTabSize(),GetVertSpac(),GetWordSpac(),m_calcVer,FileType);  
...
myBookRecord SZEbookReader::ReturnRecordByAddstus( int index,int contentSize,int fileID,int addstus ,int strsize)
{
	myBookRecord temp;
...
void SZEbookReader::SetHighLight2( SZImageBit24& imgCur,CRender* tempRender,Vt_wordinfo& words,myRGBQUAD bgColor )
{


	//区分不同contentid，因为每个content有自己的字号，样式
	...
	if (words.size()>0)
	{
		...
		for (k = 0; k < words.size(); k++)
		{
			if (words[k].contentID > contentID)
			{
				...
				int fontsize = (words[k-1].textStyle.fontSize + m_ZoomRatio) * (m_ScreenType == NormalScreen?1:ScreenRatio);
				if (fontsize < 6)
				{
					fontsize = 6;
				}
...
int SZEbookReader::SetHighLight3( SZImageBit24& imgCur,CRender* tempRender,int startContentID,int startOffset,/*int endPageId,*/int endContentID,int endOffset,myRGBQUAD bgColor,int booknoteID /*= -1*/ )
{
	...
	for (int i = 0; i < m_curPageWordInfo.lines.size();i++)
	{
		...
		for (int j = 0; j < li.words.size();j++)
		{
			...
			{
				if (wi.contentID == startContentID && wi.offset >= startOffset && wi.offset <= endOffset)
				{
					
					highlightWords.push_back(wi);
					push = true;
				}
				if (push && ((wi.contentID  == startContentID && wi.offset > endOffset) || wi.contentID > endContentID))
				{
					goto drawHightlight;
				}
			}
			...
			{                                   
				...
				if (push && (wi.contentID == endContentID && wi.offset > endOffset || wi.contentID > endContentID))
				{
					goto drawHightlight;
				}
			}
...
// 				push = true;
// 			}
			if (push && (wi.contentID >endContentID || (wi.contentID == endContentID && wi.offset > endOffset)))
			{
				goto drawHightlight;
			}			
		}
...
void* SZEbookReader::GetCoverImg(long& imageSize,int coverWidth,int coverHeight,bool isGetBookinfo)
{
	if (m_curFileType == FILETYPE_EPUB)
	{
		if (!isGetBookinfo && m_status != 1)
		{//通过getboookinfo调用的，不需要判断m_status
			return NULL;
		}
...
void* SZEbookReader::DrawCurrentPage( SZImageBit24 &imageCur,long& imageSize,t_PageOutData& pageOutData,myBookRecord* drawRecord/* = NULL*/,vector<Content*>* vcontent /*= NULL*/ )
{
	...
	{	
		Content* content = vTempContent.at(i); //(vcontent==NULL?m_currentvcontent[i]:vcontent->at(i));
		endOffset = 0;
		if (content->contentType & CT_Text)
		{
            LOGI("SZEbookReader::DrawCurrentPage | will DrawContentText");
			int rt = DrawContentText(imageCur,content,i,pageRender,offset,endOffset,pageOutData);
...
//        }
	...
	for (int j=0;j<m_bookNotes.size();j++)
	{
		...
		//pageid脢脟脣鲁脨貌脜脜脕脨碌脛隆拢鲁卢鹿媒脕脣鸥脥脥拢脰鹿
		...
		if (bookrecord.vfileID == temp.fileId)
		{
			if(SetHighLight3(imageCur,&pageRender,temp.s_contentID,temp.s_offset,temp.e_contentID,temp.e_offset,m_vColor[temp.colorIndex],temp.noteId))
			{
				curPageNote curnote;
				curnote.type = temp.type;
...
int SZEbookReader::DrawContentText( SZImageBit24 &imageCur,Content* content,int contentId,CRender& tempRender ,int offset,int& end,t_PageOutData& pageOutData)
{
	SZFTFont* pChiFtFont=NULL;//中文字体对象
	SZFTFont* pEngFtFont=NULL;//英文字体对象

	if (content == NULL)
	{
		return -1;
	}
	int fontsize = (content->textStyle.fontSize + m_ZoomRatio) * (m_ScreenType == NormalScreen?1:ScreenRatio);
	if (fontsize < 6)
	{
		fontsize = 6;
	}
	...
	int reCode = tempRender.DrawContentText(imageCur,pChiFtFont,pEngFtFont,contentId,content,offset,end,&pageOutData.pwi,&pageOutData.pmm);
	return reCode;


}

int SZEbookReader::DrawContentImage( SZImageBit24 &imageCur,Content* content,CRender& tempRender,int offset ,t_PageOutData& pageOutData,bool isRender)
{
/*图片放置规则
	1.按照等宽缩放；
	2.如果等宽之后，高度超过可绘制区域高度，那么把该图放到下一页
	3.如果这一页没有绘制过字，那么按照等高再进行缩放
*/

	//step1 得到当前页剩余可以绘制的宽高
	...
	bool NotDrawText = tempRender.GetOffsetY() == tempRender.GetMrginTop();//该页是否绘制过字。
...
// 				}
            ...
            if (pimage->GetWidth() < drawWidth && pimage->GetHeight() < drawHeight) {
                ...
                {
                    newWidth = drawWidth;
                    newHeight = (float)drawWidth/(float)pimage->GetWidth()*(float)pimage->GetHeight();
                    if (newHeight > drawHeight)
                    {
                        if (NotDrawText)
                        {//再按照等高缩放
                            newWidth = (float)drawHeight/(float)pimage->GetHeight()*(float)pimage->GetWidth();
                            newHeight = drawHeight;
                        }
                        else
                        {
...
int SZEbookReader::RenderContentEx( Vt_myBookRecord &vtCurrntFileInfo, vector<Content*> &currentvcontent,int fileID,bool isCallbackProgress/*=true*/ )
{
...
CallBack:		//回调计算进度
		...
		{
		if (m_isNeedCalcProcess && isCallbackProgress && m_bookWordTotle > 0 && content->contentType & CT_Text && content->text != NULL)
		{
...
int SZEbookReader::RendContentText( Content* content,CRender& tempRender,int begin,int& end)
{
	...
	int reCode = tempRender.DrawContentText(temp,pChiFtFont,pEngFtFont,0,content,begin,end,&tpwi,&tCmm,true);
...
void SZEbookReader::ProcessPageWordInfo( pageWordinfo& pwi )
{
	for (int i=0;i < pwi.lines.size();i++)
	{
		...
		{
			pwi.lines[i].Width = pwi.lines[i].words[pwi.lines[i].words.size()-1].x + pwi.lines[i].words[pwi.lines[i].words.size()-1].width - pwi.lines[i].x;
	...
	if (m_ScreenType == HDScreen)
	{
		for (int i=0;i < pwi.lines.size();i++)
		{
			...
			for (int j=0;j < pwi.lines[i].words.size();j++)
			{
				...
				pwi.lines[i].words[j].width = (int)((double)pwi.lines[i].words[j].width/ScreenRatio);
				pwi.lines[i].words[j].height = (int)((double)pwi.lines[i].words[j].height/ScreenRatio);
			}
		}
	}
}
...
int SZEbookReader::GetHtmlContent(int& htmlId, vector<Content* >& vContent)
{
	...
	if (m_curFileType == FILETYPE_EPUB)
	{
		while(1)
		{
			...
			{
                LOGI("SZEbookReader::GetHtmlContent| begin | will bookreader.ReadFileByFilePath m_currenthtml:%s", m_currenthtml.c_str());
				int re = bookreader.ReadFileByFilePath(m_currenthtml);
