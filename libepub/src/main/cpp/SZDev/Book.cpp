...
void CBook::ProcessHtmlCSS(bool isHaveCSS)
{
	...
	bool isP_end(false);
	bool isA(false);
	bool isOnlyImage(true);//当前html是否只有图片是有效信息
	bool isOneH(false);
	ClearVContent();
	unsigned int i,j;
	int k;
	...
	for (i=0;i<m_curFile->m_vHtmlNode.size();i++)
	{
		...
		if (htmlnode.nodeType == NODE_START_TAG)
		{
			...
			if (htmlnode.tagType == TAG_P || htmlnode.tagType == TAG_H1 || htmlnode.tagType == TAG_H2 || htmlnode.tagType == TAG_H3
				|| htmlnode.tagType == TAG_H4 || htmlnode.tagType == TAG_H5 || htmlnode.tagType == TAG_H6)
			{
		...
		else if (htmlnode.nodeType == NODE_CONTENT)
		{
			...
			for (j=0;j<v_queueBuffer.size();j++)
			{
				...
				if (isHaveCSS)
				{
					map<string,StyleSheet>::iterator pos = m_curCss->m_MCSS.find(m_ToLower(node.tagName));
 ...
 LOGI("CBook::ProcessHtmlCSS htmlnode.nodeType == NODE_CONTENT 3| i:%d",i);
				...
				for (k = 0;k < attrnum;k++ )
				{
					...
					if (isHaveCSS)
					{
						if (strcmp(node.nodeProps[k].szName,"class") == 0 || strcmp(node.nodeProps[k].szName,"id") == 0)
						{
					...
					if (strcmp(node.nodeProps[k].szName,"style") == 0)
					{
						...
						tempcss.PutData((myBYTE*)(node.nodeProps[k].szValue),strlen(node.nodeProps[k].szValue));
						tempcss.Parser();
			...
			else
			{
				...
				{
					if (temptext[after] != kongge && temptext[after] != kongge2 && temptext[after] != huanhang)
					{
						break;
					}
				}
				...
				if (after < textLegth - 1 && after >= befor)
				{
					...
					if (after < 0)
					{
						...
						memcpy(wenben->after,temptext + after,(textLegth - after - 1)*sizeof(myWcharT));
		...
		else if (htmlnode.nodeType == NODE_CLOSE_TAG)
		{
			...
			if (!v_queueBuffer.empty())
			{
				...
				if (pos.tagType == TAG_P || pos.tagType == TAG_H1 || pos.tagType == TAG_H2|| pos.tagType == TAG_H3 || pos.tagType == TAG_H4
					|| pos.tagType == TAG_H5 || pos.tagType == TAG_H6 || pos.tagType == TAG_LI || pos.tagType == TAG_DIV)
				{
					//\u5F39\u51FA\u6700\u540E\u4E00\u4E2A\u4FEE\u6539isp\u7136\u540E\u518D\u653E\u8FDB\u53BB
					if (m_vContent.size() > 0 && !isOneH/*&& (pos.tagType == TAG_P || pos.tagType == TAG_DIV)*/)
					{
						...
						for(h=0;h<tempBuffer.size();h++)
						{
							...
							if (h == 0)
							{
								...
								if (temp->before)
								{
									...
									{
										memcpy(hebingContent->before,temp->before,l*sizeof(myWcharT));
									}
								}
								else
								...
								if (temp->after)
								{
									...
									{
										memcpy(hebingContent->after,temp->after,l*sizeof(myWcharT));
									}
								}
								else
							...
							if (temp->text)
							{
								...
								memcpy(hebingContent->text+pos,temp->text,length*sizeof(myWcharT));
								pos += length;
							}
		...
		else if (htmlnode.nodeType == NODE_FULL_TAG)
		{//\u5904\u7406img
...
IMG:Content* image = new Content();
				...
				{
					if (strcmp(htmlnode.nodeProps[i].szName,"src") == 0 || strcmp(htmlnode.nodeProps[i].szName,"xlink:href") == 0)
					{
...
int CBook::parser(string& filepath)
{
	...
	if (ext == "html" || ext == "htm" || ext == "xhtml" || ext == "xml")
	{
		...
		m_FIleAnchorContentNumber.insert(pair<string,MSI>(filepath,m_curFile->GetAnchorContentNumber()));
        LOGI("CBook::parser | 2");
		if (m_curFile->IsHaveCSS())
		{
			...
			for (int i = 0; i < cssFiles.size(); i++)
			{
				...
				if (m_curCss->OpenFile(cssFiles[i].c_str(),true,false) >= 0)
                {//\u89E3\u6790\u8FC7css\u4E0D\u9700\u8981\u518D\u6B21\u89E3\u6790
...
int CBook::OpenFileByFilePath(string filepath)
{
	...
	if (er >= 0)
	{
		if (0 != otherfile->Parser())
		{
			...
			m_FIleAnchorContentNumber.insert(pair<string,MSI>(filepath,otherfile->GetAnchorContentNumber()));
	}
	delete otherfile;
	otherfile = NULL;
	return er;
