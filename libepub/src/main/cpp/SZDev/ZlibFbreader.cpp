...
int CZEpubReader::ReadFileByFilePath(string file) {
    ...
    if (-1 == re) {
        return -2;
    } else if (OPENZIPFILE_LASTOPEN == re) {//�ϴδ򿪣�����Ҫ�ٴν�����
...
int CZEpubReader::OpenTxt(char *filepath) {
    ...
    for (i = 0; i < filelength; i++) {
        ...
        {
            if (i + 3 < filelength) {
                if (tempbuff[i] == 0x0d && tempbuff[i + 1] == 0x00 && tempbuff[i + 2] == 0x0a &&
                    tempbuff[i + 3] == 0x00) {
                    isfind = true;
                    tiao = 4;
        ...
        {
            if (i + 3 < filelength) {
                if (tempbuff[i] == 0x00 && tempbuff[i + 1] == 0x0d && tempbuff[i + 2] == 0x00 &&
                    tempbuff[i + 3] == 0x0a) {
                    isfind = true;
                    tiao = 4;
        ...
        if (isfind) {
            ...
            if (encode == UTF_8) {
                strcon.Utf8ToUnicodeBig((char *) chText, length, cont->text,
                                        length * sizeof(myWcharT));
            } else if (encode == ANSI) {
                strcon.GbkToUnicodeBig((char *) chText, length, cont->text,
                                       length * sizeof(myWcharT));
            } else if (encode == UTF16_BE) {
                strcon.UnicodeSmallToUnicodeBig((char *) chText, length, cont->text,
                                                length * sizeof(myWcharT));
            } else {
#ifdef _WIN32
...
int
CZEpubReader::AutoTakeChapterinfo(EncodingType encode, Content *cont, char *filepath, int contentID,
                                  MSI &msi) {
    string strpat, re;
    strpat.assign(".*��.*��.*");
    CStringConver sc;
